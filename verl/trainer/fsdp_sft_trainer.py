...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1, global_step=None):
        ...
        # Debug printing: print model inputs/outputs every N steps
        debug_print_freq = getattr(self.config.trainer, "debug_print_freq", 0)
        should_print = (debug_print_freq > 0 and
                       global_step is not None and
                       global_step % debug_print_freq == 0 and
                       self.device_mesh.get_rank() == 0)

        if should_print:
            # Use no_grad to prevent debug operations from consuming extra memory
            with torch.no_grad():
                print("\n" + "="*80)
                print(f"[DEBUG] Global Step {global_step} - Model Input/Output")
                print("="*80)
                ...
                first_attn = attention_mask[0]
                valid_len = first_attn.sum().item()
                print(f"  Valid length: {valid_len}")
                print(f"  Input token IDs (first 50): {first_input[:50].tolist()}")

                # Decode and print the text
                try:
                    decoded_text = self.tokenizer.decode(first_input[:valid_len], skip_special_tokens=False)
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                if should_print:
                    # Use no_grad to prevent debug operations from being added to computation graph
                    with torch.no_grad():
                        print(f"\nModel output (standard mode):")
                        print(f"  logits shape: {logits.shape}")
                        print(f"  logits dtype: {logits.dtype}")
                        print(f"  logits range: [{logits.min().item():.4f}, {logits.max().item():.4f}]")
                        print(f"  logits mean: {logits.mean().item():.4f}, std: {logits.std().item():.4f}")

                        # Print predicted tokens for first sequence
                        first_logits = logits[0]  # [seq_len, vocab_size]
                        predicted_ids = first_logits.argmax(dim=-1)
                        print(f"  Predicted token IDs (first 50): {predicted_ids[:50].tolist()}")
                        try:
                            predicted_text = self.tokenizer.decode(predicted_ids[:valid_len-1], skip_special_tokens=False)
                ...
                if should_print:
                    # Use no_grad to prevent debug operations from being added to computation graph
                    with torch.no_grad():
                        print(f"\nModel output (sequence parallel mode):")
                        print(f"  output.logits shape: {output.logits.shape}")
                        print(f"  logits_rmpad shape: {logits_rmpad.shape}")
                        print(f"  logits dtype: {logits_rmpad.dtype}")
                        print(f"  logits range: [{logits_rmpad.min().item():.4f}, {logits_rmpad.max().item():.4f}]")
                        print(f"  logits mean: {logits_rmpad.mean().item():.4f}, std: {logits_rmpad.std().item():.4f}")
                        print(f"  SP rank: {self.ulysses_device_mesh.get_local_rank('sp')}/{get_ulysses_sequence_parallel_world_size()}")
            ...
            if should_print:
                # Use no_grad for printing to avoid extra memory usage
                with torch.no_grad():
                    print(f"\nLoss computation:")
                    print(f"  valid_token_this_rank: {valid_token_this_rank.item()}")
                    print(f"  dp_size: {dp_size}")
                    print(f"  raw loss (before normalization): {(loss * n_micro_batches).item():.6f}")
                    print(f"  normalized loss: {loss.item():.6f}")
                    print("="*80 + "\n")
    ...
    def training_step(self, batch: TensorDict, global_step: int = None):
        ...
        for micro_batch in micro_batches:
            loss = self._compute_loss_and_backward(batch=micro_batch, n_micro_batches=n_micro_batches, global_step=global_step)
            step_loss += loss.item()
            ...
            for step_in_epoch, data in enumerate(
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    for val_data in self.val_dataloader:
                        val_data = TensorDict(val_data, batch_size=16).to(
                            self.device_name
                        )
