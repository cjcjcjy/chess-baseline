...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # Track best validation score for saving best checkpoint
    ...
    def _save_checkpoint(self, save_type="regular"):
        """Save checkpoint.

        Args:
            save_type: Type of checkpoint to save. Options:
                - "regular": Normal checkpoint with step number (default)
                - "best": Best checkpoint based on validation score
        """
        from verl.utils.fs import local_mkdir_safe

        # path: given_path + `/global_step_{global_steps}` + `/actor` or `/best` + `/actor`
        if save_type == "best":
            local_global_step_folder = os.path.join(self.config.trainer.default_local_dir, "best")
        else:
        ...
        # For best checkpoint, always keep only 1 (the best one)
        # For regular checkpoint, use configured value
        if save_type == "best":
        ...
        if self.use_critic:
            ...
            if save_type == "best":
                critic_remote_path = (
                    None
                    if self.config.trainer.default_hdfs_dir is None
                    else os.path.join(self.config.trainer.default_hdfs_dir, "best", str(Role.Critic))
                )
            else:
                critic_remote_path = (
                    None
        ...
        # For best checkpoint, save metadata with score and step
        if save_type == "best":
            best_metadata_path = os.path.join(local_global_step_folder, "best_metadata.json")
            ...
            print(f"Saved best checkpoint at step {self.global_steps} with score {self.best_val_score:.4f}")

        # latest checkpointed iteration tracker (for atomic usage)
        # Only update for regular checkpoints, not for best checkpoints
    ...
    def _load_checkpoint(self):
        ...
        if self.config.trainer.resume_mode == "auto":
            ...
            global_step_folder = os.path.join(checkpoint_folder, "best")
            if not os.path.exists(global_step_folder):
                print("Warning: Best checkpoint not found, training from scratch")
                return 0
            # Load best checkpoint metadata
            best_metadata_path = os.path.join(global_step_folder, "best_metadata.json")
            if os.path.exists(best_metadata_path):
                with open(best_metadata_path, "r") as f:
                    best_metadata = json.load(f)
                    self.best_val_score = best_metadata.get("best_val_score", float('-inf'))
                    self.best_val_step = best_metadata.get("best_val_step", -1)
                    print(f"Loading best checkpoint with score {self.best_val_score:.4f} from step {self.best_val_step}")
        ...
        elif global_step_folder.endswith("best"):
            # For best checkpoint, try to load from metadata
            best_metadata_path = os.path.join(global_step_folder, "best_metadata.json")
            if os.path.exists(best_metadata_path):
                with open(best_metadata_path, "r") as f:
                    best_metadata = json.load(f)
                    self.global_steps = best_metadata.get("global_steps", 0)
            else:
                self.global_steps = 0
        else:
            self.global_steps = 0
    ...
    def fit(self):
        ...
        # print("Waiting for debugger attach 5679...")
        ...
        # add tqdm
        progress_bar = tqdm(total=self.total_training_steps, initial=self.global_steps, desc="Training Progress", disable=False)
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if (
                    ...
                    # Check if we should save best checkpoint based on validation score
                    if self.config.trainer.get("save_best_checkpoint", True):
                        # Extract the metric to track for "best" checkpoint
                        # Default: use specified metric key, or mean of all val-core metrics
                        best_metric_key = self.config.trainer.get("best_metric_key", None)

                        if best_metric_key and best_metric_key in val_metrics:
                            current_val_score = val_metrics[best_metric_key]
                        else:
                            # Calculate mean of all val-core metrics as default
                            core_metrics = {k: v for k, v in val_metrics.items() if k.startswith("val-core/")}
                            if core_metrics:
                                current_val_score = sum(core_metrics.values()) / len(core_metrics)
                            else:
                                current_val_score = None

                        # Save if this is the best score so far
                        if current_val_score is not None and current_val_score > self.best_val_score:
                            self.best_val_score = current_val_score
                            self.best_val_step = self.global_steps
                            print(f"New best validation score: {self.best_val_score:.4f} at step {self.best_val_step}")
                            with marked_timer("save_best_checkpoint", timing_raw, color="green"):
                                self._save_checkpoint(save_type="best")
                            metrics.update({"training/best_val_score": self.best_val_score})
